...
def _floats_feature(value): 
  #buff = struct.pack('%sf' % len(value.flatten()), *value.flatten())
  buff = value.tostring()
  buff = tf.compat.as_bytes(buff)  #works without this line, too.

  print('ok, got buff!!!!!!!!!!!!!')
  #return tf.train.Feature(float_list=tf.train.FloatList(value=value))
  #return tf.train.Feature(float_list=tf.train.FloatList(value=buff))
  return tf.train.Feature(bytes_list=tf.train.BytesList(value=buff))

def _convert_to_example(filename, image_buffer, label, label_2, text, height, width):
  ...
  example = tf.train.Example(features=tf.train.Features(feature={
      ...
      'image/class/label_2': _int64_feature(label_2),
      'image/format': _bytes_feature(tf.compat.as_bytes(k_image_format)),
      'image/filename': _bytes_feature(tf.compat.as_bytes(os.path.basename(filename))),
      ##LW
      'image/encoded': _bytes_feature(tf.compat.as_bytes(image_buffer))}))
      ##'image/dtype': _bytes_feature(image_buffer.dtype.str),
      ##'image/encoded': _floats_feature(image_buffer)  }))
  return example
...
def _process_image_files_batch(coder, thread_index, ranges, name, filenames,
                               texts, labels, labels_2, num_shards):
  ...
  for s in range(num_shards_per_batch):
    ...
    for i in files_in_shard:
      ...
      example = _convert_to_example(filename, image_buffer, label, label_2,
                                    text, height, width)
...
def _process_image_files(name, filenames, texts, labels, labels_2, num_shards):
...
# ----------------------------------
def secondaryClasseNames(fname) :
    """Returns a list of 2ndary classe names - substrings of the form ._secondaryclassname_ embedded in the file name"""
    #m = re.match('.*?(\._.*?_)\.tif$', fname)
    markedlist = re.findall('.*?(\._.*?_)', fname) # eg ['._1foo_', '._2_']
    cleanedlist = [re.match('\._(.*)_', item).group(1) for item in markedlist]
    return cleanedlist
...
def _find_image_files(data_dir, labels_file):
  ...
  labels_2 = [] # secondary label (integers)
  texts_2 = [] # secondary labels (text)
  unique_2ndary_labels = []

  # Leave label index 0 empty as a background class.
  label_index = 1


  # Construct the list of JPEG files and labels, cycling through all sounds in the MAIN class listed in the label_file, and matched by the sudirectory names
  for text in unique_labels:
    ...
    for fname in matching_files :
      scns = secondaryClasseNames(fname)
      if (len(scns) == 0) :
  ...
  texts_2 = [texts_2[i] for i in shuffled_index]
  labels_2 = [labels_2[i] for i in shuffled_index]

  #quick check on secondary labels
  #for i in range(len(filenames)) :
  #  print( texts[i] + ', ' + str(labels_2[i]))


  print('Found %d JPEG files across %d labels inside %s.' %
        (len(filenames), len(unique_labels), data_dir))
  return filenames, texts, labels, labels_2
...
def _process_dataset(name, directory, num_shards, labels_file):
  ...
  Args:
    ...
    labels_file: string, path to the labels file.
  """
  filenames, texts, labels, labels_2 = _find_image_files(directory, labels_file)
  _process_image_files(name, filenames, texts, labels, labels_2, num_shards)
